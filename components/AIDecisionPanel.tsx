...
// Removed props since we use store now
const AIDecisionPanel: React.FC = () => {
  ...
  // We don't fetch here anymore, SimulationService handles it.
  ...
  const onApply = () => {
    ...
    fetch('http://localhost:8001/api/signals/ai', {
      ...
      body: JSON.stringify({ enabled: true, scope: "GLOBAL" })
    }).catch(e => console.error("Failed to toggle AI", e));

    // Optimistic update
    if (aiStatus) {
        useSimulationStore.getState().setAIStatus({ ...aiStatus, aiActive: true });
    }
  };
      ...
      <div className="space-y-4">
        <div className="p-4 bg-slate-800/40 rounded-xl border border-slate-700/50">
          ...
          <p className="text-sm font-medium">
            {aiStatus?.prediction?.location || "Loading..."} predicted congestion in <span className="text-amber-400 font-bold">{aiStatus?.prediction?.time || 0} minutes</span>
          </p>
          <div className="mt-2 w-full h-1.5 bg-slate-700 rounded-full overflow-hidden">
            <motion.div 
              initial={{ width: 0 }}
              animate={{ width: `${Math.min(100, (aiStatus?.prediction?.time || 0) * 10)}%` }} // Arbitrary scaling
        ...
        <div className="p-4 bg-slate-800/40 rounded-xl border border-slate-700/50">
          ...
          <p className="text-sm font-medium">
             {aiStatus?.recommendation?.action || "Analyzing..."} by <span className="text-emerald-400 font-bold">{aiStatus?.recommendation?.value || ""}</span>
          </p>
          <div className="mt-2 flex items-center gap-2 text-[10px] text-emerald-400 font-bold">
            <ArrowUpRight size={12} />
            <span>Estimated Flow Improvement: {aiStatus?.efficiency || 0}%</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 mt-auto">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="flex flex-col items-center p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
            ...
            <span className="text-xs font-mono font-bold text-emerald-400">+{aiStatus?.efficiency || 0}%</span>
          </div>
        </div>
      </div>
    </div>
